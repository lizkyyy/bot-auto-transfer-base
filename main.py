...
if not w3.is_connected():
    raise Exception(Fore.RED + "Error: Unable to connect to Ethereum network!")
...
# Global settings
ETHERSCAN_API_KEY = "2ICHY4UYA84KFYNB4D3MD9Q18NNF9RE5MX"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == '1':
                gas_price_gwei = data['result']['FastGasPrice']
                print(Fore.CYAN + f"Gas price fetched: {gas_price_gwei} Gwei")
                return int(float(gas_price_gwei) * 10**9)  # Gwei to Wei
    except Exception as e:
        print(Fore.YELLOW + f"Warning: Gas price fetch failed ({e}). Using fallback gas price.")
    fallback_price = w3.eth.gas_price
    print(Fore.CYAN + f"Fallback gas price: {fallback_price} Wei")
...
def transfer_token(sender_private_key, sender_address, token_address, receiver_address, amount):
    ...
    tx = {
        ...
        "chainId": 8453,  # Add chainId here for Base network
    }
...
# Main bot logic with wallet switching
